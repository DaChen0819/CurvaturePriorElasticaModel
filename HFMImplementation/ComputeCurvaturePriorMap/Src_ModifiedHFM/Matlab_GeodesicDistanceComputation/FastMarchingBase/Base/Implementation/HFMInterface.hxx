...
Run_SetupSolver() {
    ...
    if(io.HasField("mask"))
        pFM->activeRegion = io.GetArray<ScalarType, Dimension>("mask");
    else if(io.HasField("activeRegion"))
        pFM->activeRegion = io.GetArray<ScalarType, Dimension>("activeRegion");
...
Run_ExtractGeodesics() {
    ...
    if(io.HasField("tips")){
        ...
        for(PointType & tip : tips){
            ...
            if(!pFM->dom.Periodize(tipIdx,tipIdx).IsValid()){
