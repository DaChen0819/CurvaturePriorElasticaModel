...
GeodesicDiscreteSolver<Traits>::Setup(HFMI * that) {
    ...
    if(io.HasField("geoInformation")){
        outputInformation=io.template Get<ScalarType>("geoInformation")>0.5;
    }
    else{
...
GeodesicDiscreteSolver<Traits>::Run(HFMI * that, const std::vector<PointType> & tips) {
    ...
    for(const PointType & tip : tips){
        ...
        if(failed && outputInformation)
            Msg() << "Geodesic extraction failed for tip "
			<< that->stencil.Param().ReDim( tip ) << "\n";
        else if(nRestarts>nRestartsBeforeVolumeIncrease && outputInformation){
...
Run(std::vector<PointType> & geodesic) const  {
    if(geodesic.empty())
        ExceptionMacro("GeodesicDiscreteSolver error : no start point !");
        nRestarts = 0;
