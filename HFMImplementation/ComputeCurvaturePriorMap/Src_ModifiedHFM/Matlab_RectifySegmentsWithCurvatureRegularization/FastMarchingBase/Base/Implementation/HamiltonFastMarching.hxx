...
HamiltonFastMarching<T>::ConditionalUpdate(IndexCRef acceptedIndex,
    ...
    PhysicalIndexType PhysicalSpatialIndex;
      for(int k=0;k<Dimension-1;k++)
          PhysicalSpatialIndex[k]=updated.index[k];
      
    if(!physicalActiveRegion.empty() && physicalActiveRegion(PhysicalSpatialIndex)<0.5)
            return;
